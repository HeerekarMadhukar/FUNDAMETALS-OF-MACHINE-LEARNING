...
library(tidyverse)
library(ISLR)
pharmacy <- read.csv("C://Users//heere//OneDrive//Desktop/FML ASSIGNMENT 4//pharmacy.csv")
View(pharmacy)
#TASK1
...
Pharmacy1 <- x[,3:11]
head(Pharmacy1)
Pharmacy2 <- scale(Pharmacy1)
head(Pharmacy2)

fviz_nbclust(Pharmacy2, kmeans, method = "wss") + labs(subtitle = "Elbow Method")
fviz_nbclust(Pharmacy2, kmeans, method = "silhouette") + labs(subtitle = "Silhouette Method")

set.seed(64060)

k5 <- kmeans(Pharmacy2, centers = 5, nstart = 25)
k5$centers
fviz_cluster(k5, data = Pharmacy2)

distance <- dist(Pharmacy2, method = "euclidean")
fviz_dist(distance)

fit <- kmeans(Pharmacy2, 5)
aggregate(Pharmacy2, by=list(fit$cluster), FUN=mean)
pharmacy3 <- data.frame(Pharmacy2, fit$cluster)
pharmacy3
library(cluster)
clusplot(Pharmacy2, fit$cluster, color = TRUE, shade = TRUE,
         labels = 2, lines = 0)
...
#Cluster1-- Profitable clusture
#cluster2-- Gold mine clusture
#cluster3-- primary clusture
#clusture4-- Bear clusture
#clusture5--  continuing clusture
